  ...
  return (
    <Canvas camera={{ fov: 12, position: [5, 0.6, 15] }}>
      <Suspense maxDuration={5000} fallback={null}>
        ...
        />
        <Ocean />
        <Wave file={[require("./posters/elaine.jpg")]} position={0} planeGeometryArgs={[0.4, 0.4 * (8 / 5), 16, 16]} />
        <Wave file={[require("./posters/elaine2.jpg")]} position={0.65} planeGeometryArgs={[0.5, 0.5 * (8 / 5), 16, 16]} />
        <Wave file={[require("./posters/elaine3.jpg")]} position={-0.65} planeGeometryArgs={[0.6, 0.6 * (8 / 5), 16, 16]} />

        <group transform position={(0, 0, 0.5)}>
          {props.visiblePhotos.map((item, index) => (
            <Wave key={index} file={item.file} position={-1.5 + index / 2} planeGeometryArgs={[0.4, 0.4 * (4 / 6), 16, 16]} />
          ))}
        </group>
